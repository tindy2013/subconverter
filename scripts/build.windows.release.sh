...
cd curl
cmake -DCMAKE_BUILD_TYPE=Release -DHTTP_ONLY=ON -DCURL_USE_SCHANNEL=ON -DBUILD_SHARED_LIBS=OFF -DBUILD_CURL_EXE=OFF -DCMAKE_INSTALL_PREFIX="$MINGW_PREFIX" -G "Unix Makefiles" -DUSE_LIBSSH2=OFF -DUSE_LIBIDN2=OFF -DCURL_USE_LIBPSL=OFF -DUSE_ZSTD=ON -DUSE_BROTLI=ON -DUSE_NGHTTP2=OFF .
make install -j4
cd ..
